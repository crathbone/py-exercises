...
def problem1_3(n):
    ct=0
    for ct in range(n,0,-1):  #generates a list of n numbers - for 
#        ct=sum(n)
 #       sum(0,n)
 #       ct in range(n,0,-1):
        print(ct)  #print(ct) prints n numbers
                   #sum(ct) does not work
#FIXIT - this is not working yet; note that print is not the expected output
#    ct=1
#    while ct <= n:
#        for n in range(1,n,1):
#            sum = (n+1)
#            print(sum,end=" ")
            #no good, endless loop
#%%
"""
...
def problem1_4(miles):
    miles_str=int(miles)
    feet=(miles_str*5280)
    print("There are",feet, "feet in",miles_str,"miles.")
#CR this solution works   
#%%
"""
Test run. Note that the grader program will use different numbers:
...
def problem1_6():
    n=1
    for ct in range(n,100,2):  #when n=1, generates a list of odd numbers - for ct in range(n,100,2):
        print(ct,end=" ")  #print(ct,end=" ") prints n numbers
#CR this solution works   
#%% 
"""
...
#Area = (1/2)(b1+b2)h
    area=.5*(x+y)*z
    print("The area of a trapezoid with bases",x,"and",y,"and height",z,"is",area)
#Cr this solution works
#%%
"""
